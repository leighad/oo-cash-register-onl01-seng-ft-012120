describe 'CashRegister' do
  ...
  xdescribe '#remove_last_item' do
    it 'accepts a title and a price and decreases the total' do
      expect{cash_register.remove_last_item("eggs", 0.98)}.to change{cash_register.total}.by(-0.98)
    end
  
    it 'also accepts an optional quantity' do
      expect{cash_register.remove_last_item("book", 5.00, 3)}.to change{cash_register.total}.by(-15.00)
    end

# string = "string"
# expect { string }.not_to change { string }.from("string")
  
    it "doesn't forget about the previous total" do
      cash_register.remove_last_item("Lucky Charms", 4.5)
      # expect(cash_register.total).to eq(4.5)
      expect(cash_register.total).to eq(-4.5)
      cash_register.remove_last_item("Ritz Crackers", 5.0)
      # expect(cash_register.total).to eq(9.5)
      expect(cash_register.total).to eq(-9.5)
      cash_register.remove_last_item("Justin's Peanut Butter Cups", 2.50, 2)
      # expect(cash_register.total).to eq(14.5)
      expect(cash_register.total).to eq(-14.5)
    end
  end
